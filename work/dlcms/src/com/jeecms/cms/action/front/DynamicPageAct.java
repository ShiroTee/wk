	...
	public String dynamic(HttpServletRequest request,
		...
		if (len == 1) {
			...
			if (paths[1].equals(INDEX)) {
				String responseStr = channel(paths[0], pageNo, params, info, request,
						response, model) ;
//				如果点击的是“我的服务”页面，则需要验证用户是否登陆。否则跳转到“会员登陆页面.html”--zhyg
				if("myService".equals(paths[0])){
//					获取用户登陆信息--zhyg
					CmsUser user = CmsUtils.getUser(request);
					if(user == null){
						model.addAttribute(MESSAGE, "true");//提示用户需要登陆才能进行操作
						model.addAttribute(RETURN_URL, model.get("location"));
						responseStr = "/WEB-INF/t/cms/www/red/member/会员登录页.html" ;
					}
				}
				// 栏目页
				return responseStr;
