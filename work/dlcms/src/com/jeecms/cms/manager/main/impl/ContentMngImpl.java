...
public class ContentMngImpl implements ContentMng, ChannelDeleteChecker {
	...
	public Content save(Content bean, ContentExt ext, ContentTxt txt,
		...
		//处理类型ID为空时，不能添加的问题，例如:数据成果模块，内容里没有类型为null,则typeId也为null
		typeId = typeId == null ? 0 : typeId ;	//如果为null,默认将内容类型设置为0，类型为“无”
		bean.setType(typeId != null ? contentTypeMng.findById(typeId) : null);
		bean.setUser(user);
		Byte userStep;
		if (forMember) {
			// 会员的审核级别按0处理
