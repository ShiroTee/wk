...
public class AdminContextInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		//查询在session中是否有RDP平台的用户已登陆。
		UnifiedUser rdpUser = authMng.retrieveRdpUserFromSession(session, request, (String)session.getAttribute(request, "auth_key"));
		// 获得用户
		CmsUser user = null;
		if (adminId != null) {
			// 指定管理员（开发状态）
		...
		// 用户为null跳转到登陆页面
	/*	if(rdpUser != null && rdpUser.getRdpUserId() != null){
			...
			return false;
		//user为空表示没有门户自身用户登陆，再判断是否有RDP平台用户登陆，如果有则提示无权限
		}
		// 用户不是管理员，提示无权限。
		/*if (!user.getAdmin()) {
			request.setAttribute(MESSAGE, MessageResolver.getMessage(request,
