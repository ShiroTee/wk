...
public class IRCServInfoAct{
	...
	@RequestMapping(value = "/irc/irc_servinfo.jspx", method = RequestMethod.POST)
	protected String getIRCServerInfo(String returnUrl, HttpServletRequest request,
			HttpServletResponse response, ModelMap model){
		//错误编码：irc005:获取服务详情失败;irc006:具体服务ID未找到;
				//获取传入参数
				JSONObject responseData = new JSONObject() ;
				String resid = RequestUtils.getQueryParam(request, "id");
				//String resType = RequestUtils.getQueryParam(request, "servType");
				
				if(!StringUtils.isBlank(resid)){
					String getServInfo = "/app/api/service/irc/ircIrcDataDealHandler/findResourceDetail?resid=" + resid ;
					//测试固定ID
					//String getServInfo = "/common/ircresdata.jsp?method=JsonResDetail&resid=" + "41556b13cd0010a87611b3f114a85344" ;
					//getServInfo += "&resType=" + resType ;
					try {
						responseData = new JSONObject(RdpUtils.readRdpInterface(request, getServInfo, "json")) ;
						ResponseUtils.renderJson(response, responseData.toString()) ;
					} catch (Exception e) {
						//irc005:根据树节点ID从IRC平台获取资源服务列表失败;
						ResponseUtils.renderJson(response, "{'success':false,'error':'irc005'}") ;
						e.printStackTrace();
					}
				}else{
					//irc006:具体服务ID未找到。
					ResponseUtils.renderJson(response, "{'success':false,'error':'irc006'}") ;
				}
