...
Ext.onReady(function()
{
	...
	[
			'WBJNAME','JOBNAME','BATCHID', 'CHANNELID', 'LOGDATE', 'TRANSNAME', 'STEPNAME', 'STEPCOPY', 'LINESREAD', 'LINESWRITTEN', 'LINESUPDATED', 'LINESINPUT', 'LINESOUTPUT', 'LINESREJECTED', 'ERRORS'
	];
		...
		} else
		{
			...
			Ext.Ajax.request(
			{
				...
				success : function(response, options)
				{
					...
					if(result[0].LOGFIELD !=null){
						Ext.getCmp('LOGFIELD').setValue(result[0].LOGFIELD);
					}else{
						Ext.getCmp('LOGFIELD').setValue(" ");
					}
	...
	var toolBar = new Ext.Toolbar(
	{
		...
		[
				...
				}, '->',
				{
					...
					handler : function()
					{
						var msg = '步骤执行情况用于对kettle中所有的step操作信息进行监控，step是转换（transformation）中的一个操作步骤';
...
//					},
	...
	/** *****以下为注册各页面组件方法******************************* */
	// 提示标签提示的内容;
	setTip("对步骤的今日和历史执行情况进行展示，点击每条记录的日志详情，可查看该转换运行的详细日志信息。");
