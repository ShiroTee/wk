...
import com.digitalchina.ldp.app.dms.service.CleanMonitorService;
...
@Component
public class CleanMonitorHandler extends AbstractExtHandler
{

	@Autowired
	private CleanMonitorService	cleanMonitorService;

	@Override
	public ViewModel page(Model model)
	{
		ViewModel viewModel = new ViewModel("cleanmonitor/cleanmonitor.jsp", "flowfollow/dynamicGrid.js");
		...
		model.getRequest().setAttribute("tipMsg", "监控数据清洗过程，双击列表记录，可下钻查看该作业的转换、步骤的执行情况。");
		return viewModel;
	}
	...
	{
		List<RDirectoryBean> list = cleanMonitorService.getDirectoryList();
	...
	{
		List<String> list = cleanMonitorService.queryLogMessage(model);
		return list;
	}
	...
	private Map<String, Object> getPageExt(String queryColParam)
	{
		...
		if (("jobGo").equals(queryColParam))
		{
			...
			columnNamesList.add("{fieldsValue: \"{name: 'JOBNAME'}\", columnsValue: \"{header: '作业名称', dataIndex: 'JOBNAME', width: 130, align: 'center', sortable: true, summaryType: 'count', summaryRenderer: summaryRenderer, renderer: formatQtip}\"}");
			columnNamesList.add("{fieldsValue: \"{name: 'GOVNAME'}\", columnsValue: \"{header: '所属委办局', dataIndex: 'GOVNAME', width: 90,  sortable: true, renderer: formatQtip}\"}");
		...
		else if (("transGo").equals(queryColParam))
		{
			...
			columnNamesList.add("{fieldsValue: \"{name: 'BATCHID'}\", columnsValue: \"\"}");
			columnNamesList.add("{fieldsValue: \"{name: 'CHANNELID'}\", columnsValue: \"\"}");
			columnNamesList.add("{fieldsValue: \"{name: 'TRANSNAME'}\", columnsValue: \"{header: '转换名称', dataIndex: 'TRANSNAME', width: 120, align: 'center', sortable: true, summaryType: 'count', summaryRenderer: summaryRenderer, renderer: formatQtip}\"}");
		...
		else if (("stepGo").equals(queryColParam))
		{
			...
			columnNamesList.add("{fieldsValue: \"{name: 'BATCHID'}\", columnsValue: \"\"}");
			columnNamesList.add("{fieldsValue: \"{name: 'CHANNELID'}\", columnsValue: \"\"}");
			columnNamesList.add("{fieldsValue: \"{name: 'STEPNAME'}\", columnsValue: \"{header: '步骤名称', dataIndex: 'STEPNAME', width: 350, align: 'center', sortable: true, summaryType: 'count', summaryRenderer: summaryRenderer, renderer: formatQtip}\"}");
	...
	public Map<String, Object> getColumnName(Model model)
	{
		...
		{
			boolean isHaveTrans = cleanMonitorService.checkIsHaveTrans(model);
			if (isHaveTrans)
			{
				// 若有TRANS,则进入TRANS
		...
		{
			// 校验是否还有子STEP
			boolean isHaveStep = cleanMonitorService.checkIsHaveStep(model);
			if (isHaveStep)
			{
				// 若有STEP,则进入STEP
	...
	private SpecilPageList<String> getQryList(String queryColParam, Model model)
	{
		...
		if (("jobGo").equals(queryColParam))
		{
			specilPageList = cleanMonitorService.getJobData(start, end, model);
		}
		if ("transGo".equals(queryColParam))
		{
			specilPageList = cleanMonitorService.getTransData(start, end, model);
		}
		if ("stepGo".equals(queryColParam))
		{
			specilPageList = cleanMonitorService.getStepData(start, end, model);
		}
