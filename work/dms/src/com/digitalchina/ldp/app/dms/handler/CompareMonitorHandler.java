...
import com.digitalchina.ldp.app.dms.service.CompareMonitorService;
...
@Component
public class CompareMonitorHandler extends AbstractExtHandler
{
	...
	{
		ViewModel viewModel = new ViewModel("comparemonitor/comparemonitor.jsp", "flowfollow/dynamicGrid.js");
		...
		model.getRequest().setAttribute("tipMsg", "监控数据比对过程，双击列表记录，可下钻查看该作业的转换、步骤的执行情况。");
		return viewModel;
	}
	...
	{
		List<RDirectoryBean> list = compareMonitorService.getDirectoryList();
	...
	{
		List<String> list = compareMonitorService.queryLogMessage(model);
		return list;
	}
	...
	public Map<String, Object> getColumnName(Model model)
	{
		...
		{
			boolean isHaveTrans = compareMonitorService.checkIsHaveTrans(model);
			if (isHaveTrans)
			{
				// 若有TRANS,则进入TRANS
		...
		{
			// 校验是否还有子STEP
			boolean isHaveStep = compareMonitorService.checkIsHaveStep(model);
			if (isHaveStep)
			{
				// 若有STEP,则进入STEP
	...
	private SpecilPageList<String> getQryList(String queryColParam, Model model)
	{
		...
		if (("jobGo").equals(queryColParam))
		{
			specilPageList = compareMonitorService.getJobData(start, end, model);
		}
		if ("transGo".equals(queryColParam))
		{
			specilPageList = compareMonitorService.getTransData(start, end, model);
		}
		if ("stepGo".equals(queryColParam))
		{
			specilPageList = compareMonitorService.getStepData(start, end, model);
		}
