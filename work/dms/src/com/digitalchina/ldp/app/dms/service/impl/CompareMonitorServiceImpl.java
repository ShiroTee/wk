...
import com.digitalchina.ldp.app.dms.dao.CompareMonitorDao;
...
@Service
public class CompareMonitorServiceImpl implements CompareMonitorService
{

	@Autowired
	private CompareMonitorDao	compareMonitorDao;
	...
	public List<RDirectoryBean> getDirectoryList()
	{
		...
		List<RDirectoryBean> list = compareMonitorDao.find(RDirectoryBean.class, argsMap);
		return list;
	}
	...
	public boolean checkIsHaveTrans(Map<String, Object> map)
	{
		...
		if (paramSql != "")
		{
			...
			sql.append(paramSql);
			count = compareMonitorDao.getTotal(sql.toString());
		}
		if (count == 0)
		{
			flag = false;
	...
	public SpecilPageList<String> getJobData(int start, int end, Map<String, Object> map)
	{
		...
		sql.append("         AND A.STEPNAME = '比对' ");
		sql.append(qrySql);
		sql.append(" ORDER BY TO_CHAR (L.LOGDATE, 'YYYY-MM-DD HH24:MI:SS') DESC,R.NAME asc ");
		List list = compareMonitorDao.findByPage(start, end, sql.toString());
		...
		sqlCount.append("         AND A.STEPNAME = '比对' ");
		sqlCount.append(qrySql);
		int count = compareMonitorDao.getTotal(sqlCount.toString());
	...
	public List<String> queryLogMessage(Map<String, Object> map)
	{
		...
		if (channelId != "" && stepType != "")
		{
			...
			sql.append(" WHERE 1 = 1 ");
			sql.append(qrySql);
			List list = compareMonitorDao.findBySql(sql.toString());
