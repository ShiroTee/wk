...
@Service
public class CleanMonitorServiceImpl implements CleanMonitorService
{

	@Autowired
	private CleanMonitorDao	cleanMonitorDao;
	...
	public boolean checkIsHaveTrans(Map<String, Object> map)
	{
		...
		if (paramSql != "")
		{
			...
			sql.append(paramSql);
			count = cleanMonitorDao.getTotal(sql.toString());
		}
		if (count == 0)
		{
			flag = false;
	...
	public SpecilPageList<String> getJobData(int start, int end, Map<String, Object> map)
	{
		...
		sqlCount.append("         AND A.STEPNAME = '清洗' ");
	...
	public SpecilPageList<String> getTransData(int start, int end, Map<String, Object> map)
	{
		...
		// 计数的SQL
		StringBuffer sqlCount = new StringBuffer();
		sqlCount.append(" SELECT DISTINCT R.CHANNEL_ID CHANNELID, T.ID_JOB JOBID, R.BATCH_FLAG BATCHFLAG ");
