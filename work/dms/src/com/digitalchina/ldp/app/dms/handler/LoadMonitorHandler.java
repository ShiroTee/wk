...
@Component
public class LoadMonitorHandler extends AbstractExtHandler
{
	...
	{
		ViewModel viewModel = new ViewModel("loadmonitor/loadmonitor.jsp", "flowfollow/dynamicGrid.js");
		...
		model.getRequest().setAttribute("tipMsg", "监控数据加载过程，双击列表记录，可下钻查看该作业的转换、步骤的执行情况。");
		return viewModel;
	}
	...
	{
		List<RDirectoryBean> list = loadMonitorService.getDirectoryList();
	...
	{
		List<String> list = loadMonitorService.queryLogMessage(model);
		return list;
	}
	...
	public Map<String, Object> getColumnName(Model model)
	{
		...
		{
			boolean isHaveTrans = loadMonitorService.checkIsHaveTrans(model);
			if (isHaveTrans)
			{
				// 若有TRANS,则进入TRANS
		...
		{
			// 校验是否还有子STEP
			boolean isHaveStep = loadMonitorService.checkIsHaveStep(model);
			if (isHaveStep)
			{
				// 若有STEP,则进入STEP
	...
	private SpecilPageList<String> getQryList(String queryColParam, Model model)
	{
		...
		if (("jobGo").equals(queryColParam))
		{
			specilPageList = loadMonitorService.getJobData(start, end, model);
		}
		if ("transGo".equals(queryColParam))
		{
			specilPageList = loadMonitorService.getTransData(start, end, model);
		}
		if ("stepGo".equals(queryColParam))
		{
			specilPageList = loadMonitorService.getStepData(start, end, model);
		}
