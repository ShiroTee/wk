...
import com.digitalchina.ldp.app.dms.dao.TransMonitorDao;
...
public class TransMonitorServiceImpl implements TransMonitorService
{
	...
	public List<RDirectoryBean> getDirectoryList()
	{
		...
		List<RDirectoryBean> list = transMonitorDao.find(RDirectoryBean.class, argsMap);
		return list;
	}
	...
	public boolean checkIsHaveTrans(Map<String, Object> map)
	{
		...
		if (paramSql != "")
		{
			...
			sql.append(paramSql);
			count = transMonitorDao.getTotal(sql.toString());
		}
		if (count == 0)
		{
			flag = false;
	...
	public SpecilPageList<String> getJobData(int start, int end, Map<String, Object> map)
	{
		...
		sql.append("         AND A.STEPNAME = '转换' ");
		sql.append(qrySql);
		sql.append("ORDER BY TO_CHAR (L.LOGDATE, 'YYYY-MM-DD HH24:MI:SS') DESC,R.NAME asc ");
		List list = transMonitorDao.findByPage(start, end, sql.toString());
		...
		sqlCount.append("         AND A.STEPNAME = '转换' ");
		sqlCount.append(qrySql);
		int count = transMonitorDao.getTotal(sqlCount.toString());
	...
	public List<String> queryLogMessage(Map<String, Object> map)
	{
		...
		if (channelId != "" && stepType != "")
		{
			...
			sql.append(" WHERE 1 = 1 ");
			sql.append(qrySql);
			List list = transMonitorDao.findBySql(sql.toString());
