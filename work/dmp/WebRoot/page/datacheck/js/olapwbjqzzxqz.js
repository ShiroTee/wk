...
var queryListUrl = REQUEST_URL_BASE + "olapWbjqzZxqzaHandler/geOlapWbjqzZxqzaList";
//创建服务基本信息表单面板
var LABEL_WIDTH = 340;
var anchorShow='100%';
Ext.onReady(function() {
		...
		Ext.override(Ext.menu.Menu,
	{
			...
			listeners :
			{
				select :  function(serviceInterfaceCombox,
						record, index)
						{
							...
							var combo1 = Ext.getCmp("jckNameValueto");
							//根据选择的服务接口 加载服务接口下的方法
							var combo = Ext.getCmp("jckTableNamesto");
							combo.store.load((
							{
								params :
								{
		...
		/** *****以下为注册各页面组件方法******************************* */
		// 提示标签提示的内容;
		setTip("按时间段统计显示委办局前置和中心前置数据对帐的结果,如果数据有较大差异,用红色字体显示,差异较小用蓝色字体显示.");
		...
		setMainPanel("wbjqzzxqz_div_1", grid);
