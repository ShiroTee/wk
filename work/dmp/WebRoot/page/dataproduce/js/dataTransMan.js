...
var cleanRuleJsonStoreURL = REQUEST_URL_BASE
		+ "transDataManHandler/getDataRule";
// 数据转换结果统计
var queryListUrl = REQUEST_URL_BASE + "transDataManHandler/getTransResult";
// 根据转换代号查询异常数据需要显示的字段及字段名
var fieldAndFieldnameUrl = REQUEST_URL_BASE
		+ "transDataManHandler/getErrorFiledAndName";
// 查询异常数据详情
var errorListUrl = REQUEST_URL_BASE
		+ "transDataManHandler/getOneDayFailedInfo";
var anchorShow='100%';
var oneMonthTable;
		...
		Ext.override(Ext.menu.Menu,
	{
		autoHeight : function()
		{
			...
			var gridStoreFields_datatrans = [ 'DATASOURCE', 'DATARULECODE',
					'DATARULENAME','COLUMNNAMEVALUE','TABLENAMEVALUE','TABLENAME', 'INSERTDATE', 'SUCCESS', 'FAILED',
			...
			function renderOper_datatrans(value, metadata, record, rowIndex,
				...
				if (record.get('FAILED') != 0) {
					result = "<font color='blue'><a href='javascript:void(0)' onclick='showMore_datatrans(\""
...
//				if (record.get('FAILED') != 0) {
//					showMore_datatrans(record.get('DATARULECODE'),record
...
//			});
			...
			function queryFunc_datatrans() {
				...
				gridStore_datatrans.baseParams = queryForm_datatrans.getForm()
						.getValues();
			...
			showMore_datatrans = function(TABLENAME,DATARULECODE,TABLENAMEVALUE,COLUMNNAMEVALUE , INSERTDATE) {
				// 动态生成表格字段
				Ext.Ajax
						.request({
							...
							success : function(responseObject_datatrans) {
								...
								//异常详情“异常字段”格式化
//								function failedOper_datatrans(value, metadata, record, rowIndex,colIndex, store) {
//									return '<span style="color:red;">' + value + '</span>' ;
//								}
...
//										html : "<span style='font-weight:bold;margin-left: 5px;'>异常数据详情</span>"
//									}), new Ext.Button({
//										text : "&nbsp;&nbsp返&nbsp;回&nbsp;&nbsp;",
//										type:"button",
								...
								var errorInfoGrid_datatrans = new Ext.grid.GridPanel(
										{
											...
											buttonAlign : 'center',
											autoWidth : true,
											height : grid_datatrans.getHeight(),
											frame : false,
...
//								errorInfoGrid_datatrans.render('content_5');
								...
								// 显示窗口
								detailWindow.setTitle(TABLENAME+"数据转换异常数据详情");
			...
			/** *****以下为注册各页面组件方法******************************* */
			// 提示标签提示的内容;
			setTip("按时间段统计各委办局数据转换成功记录总数和不满足转换规则的记录总数,点击异常数据详情后,可查看异常数据的异常字段,用红色字体显示.");
			// 构建一个查询面板，参数依次为：面板的ID、面板里的元素组、点查询按钮时触发的函数
			queryForm_datatrans = setQueryForm("queryForm_datatrans",
					queryCodeFormItems_datatrans, queryForm_datatrans);
			// 根据ID获取组件。例如获取查询面板组件
