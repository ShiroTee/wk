...
import com.digitalchina.ldp.app.dmp.service.ITransDataManService;
...
@Component
public class TransDataManHandler extends AbstractExtHandler {

	@Autowired
	private ITransDataManService transDataManService ;

	//根据委办局别名、表名、字段名，查询出此字段相应的清洗规则集合
	public String getDataRule(Model argsMap){
		List<DataConfigBean> list = transDataManService.getDataRule(argsMap) ;
		CreateJson resJson = new CreateJson() ;
		for(DataConfigBean dataConfig: list){
			if(dataConfig.getDataRuleCode().startsWith("T_")){
				resJson.add("DataRuleCode", dataConfig.getDataRuleCode()) ;
				resJson.add("DataRuleName", dataConfig.getDataRuleName()) ;
				resJson.addToList() ;
			}
		}
		return resJson.getResultJson() ;
	}
	//根据清洗规则查询出某时间段里所有清洗统计结果
	public PageList<Map<String, Object>> getTransResult(Model argsMap) {
		String startDate = argsMap.getValue("starttime_date");
		String endDate = argsMap.getValue("endtime_date");
		PageList<Map<String, Object>> pageList = transDataManService.getTransResultInfo(argsMap) ;
	...
	public String getErrorFiledAndName(Model argsMap){
		...
		if(!StringUtils.isBlank(DataRuleCode)){
			respList = transDataManService.getErrorFieldAndName(argsMap, DataRuleCode) ;
			//查询到字段和字段名数据
			if(respList.size() > 0 ){
...
//				map4.put("renderer", "failedOper_dataclean") ;
	...
	public PageList<Map<String, Object>> getOneDayFailedInfo(Model argsMap) {
		...
		list = transDataManService.getOneDayFailedInfo(argsMap);
	...
	{
		ViewModel viewModel=new ViewModel("dataproduce/dataTransMan.jsp");
		return viewModel;
	}
}
