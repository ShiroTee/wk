...
@Service
public class TransDataManServiceImpl implements ITransDataManService {
	...
	public PageList<Map<String, Object>> getTransResultInfo(Model argsMap, Object... args1) {
		String SQLStr1 = "SELECT d.TABLENAMEVALUE,d.TABLENAME, d.COLUMNNAMEVALUE, d.datasourcevalue,d.datasource,ol.datarulecode,d.datarulename,NVL(ol.success,0) success,NVL(ol.failed,0) failed,ol.insertdate FROM olap_transition_count ol INNER JOIN DMP_DATACONFIG d ON ol.datarulecode=d.datarulecode WHERE 1=1 ";
...
//		如果字段存在规则存在
			...
			argsList.add("T_" + wbjCode + "_" + tableNameEn + "_" + columnNameEn + "%") ;
			countArgsList.add("T_" + wbjCode + "_" + tableNameEn + "_" + columnNameEn + "%") ;
//		如果表名存在
		}else if(!StringUtils.isBlank(tableNameEn)){
			sql.append("AND ol.datarulecode LIKE ? ") ;
			argsList.add("T_" + wbjCode + "_" + tableNameEn + "%") ;
			countArgsList.add("T_" + wbjCode + "_" + tableNameEn + "%") ;
//		如果委办局存在
		}else if(!StringUtils.isBlank(wbjCode)){
			sql.append("AND ol.datarulecode LIKE ? ") ;
			argsList.add("T_" + wbjCode + "%") ;
			countArgsList.add("T_" + wbjCode + "%") ;
		}
