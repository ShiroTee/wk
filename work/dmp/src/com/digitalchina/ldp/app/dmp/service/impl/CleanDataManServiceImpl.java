...
@Service
public class CleanDataManServiceImpl implements ICleanDataManService {
	...
	public PageList<Map<String, Object>> getCleanResultInfo(Model argsMap, Object... args1) {
		String SQLStr1 = "SELECT d.name,DM.EXCEPTIONBM,d.TABLENAMEVALUE,d.TABLENAME, d.COLUMNNAMEVALUE,d.datasourcevalue,d.datasource,ol.datarulecode,d.datarulename,NVL(ol.success,0) success,NVL(ol.failed,0) failed,ol.insertdate FROM olap_clean_count ol INNER JOIN DMP_DATACONFIG d ON ol.datarulecode=d.datarulecode JOIN DMP_WBJ_TABLE DM  ON DM.BM = D.TABLENAMEVALUE WHERE 1=1 ";
...
//		如果字段存在规则存在
			...
			argsList.add("C_" + wbjCode + "_" + tableNameEn + "_" + columnNameEn + "%") ;
			countArgsList.add("C_" + wbjCode + "_" + tableNameEn + "_" + columnNameEn + "%") ;
//		如果表名存在
		}else if(!StringUtils.isBlank(tableNameEn)){
			sql.append("AND ol.datarulecode LIKE ? ") ;
			argsList.add("C_" + wbjCode + "_" + tableNameEn + "%") ;
			countArgsList.add("C_" + wbjCode + "_" + tableNameEn + "%") ;
//		如果委办局存在
		}else if(!StringUtils.isBlank(wbjCode)){
			sql.append("AND ol.datarulecode LIKE ? ") ;
			argsList.add("C_" + wbjCode + "%") ;
			countArgsList.add("C_" + wbjCode + "%") ;
		}
...
//		判断日期是否存在
	...
	public PageList<Map<String, Object>> getOneDayFailedInfo(Model argsMap,
		...
		//异常表表名
		String ExceptionBm=argsMap.getValue("ExceptionBm");
		...
		String ColumnNameValue="COLUMNNAMEVALUE";
		...
		String exceptionTable = ExceptionBm+Constant.DBLINK ;
		String SQLStr1 = "SELECT ex.id,d.tablename,d.tablenamevalue,D." ;
		String SQLStr2 = ",ex.excepition_info,ex.doflag,ex.dotime,ex.intime FROM " ;
		String SQLStr3 = " ex INNER JOIN DMP_DATACONFIG d ON EX.EXCEPITION_INFO = D.NAME WHERE ex.doflag='N' AND ex.Excepition_Info=? AND trunc(ex.intime)<＝to_date(?,'yyyy-MM-dd') " ;
		String sql = SQLStr1 + ColumnNameValue.toLowerCase() + SQLStr2 + exceptionTable + SQLStr3 +" AND d.tablenamevalue='"+TableNameValue+"' AND ID IS NOT NULL AND INTIME IS NOT NULL ";
		
		int count = wBJTableInfoDao.getCount(sql, Name, insertDate) ;
		List<Map<String, Object>> list = null;
		list = wBJTableInfoDao.getTableNameListGroupByBM(sql, Name, insertDate, start, pageSize) ;
//		格式化异常数据列，使其显示为红色
