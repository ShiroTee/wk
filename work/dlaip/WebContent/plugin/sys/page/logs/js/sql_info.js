...
function createGrid() {
		...
		handler : function() {
			Ext.MessageBox.confirm("提示", "你确定要清理所有数据吗?", function(btnId) {
				if (btnId == 'yes') {
					...
					var url = getHandlerRequestUrl() + "dataSourceInfoHandler/clear?type=1";
					Ext.MessageBox.confirm("提示", "你要导出sql数据吗?", function(btnId) {
					if (btnId == 'yes') {
						window.location.href = getHandlerRequestUrl() + "dataSourceInfoHandler/clear?type=2";
					}else{
						Ext.Ajax.request({
							url : url,
							method : 'POST',
	...
	toolbar.addButton({
		...
		iconCls : 'icon_export',
		handler : function() {
			window.location.href = getHandlerRequestUrl() + "dataSourceInfoHandler/clear?sqlType="+Ext.getCmp("sqlType_combo").getValue();
		}
	});
	...
	var gridStore = new Ext.data.JsonStore({
		...
		extraParams:{'start':0,'limit':pageSize},
		...
		url : getHandlerRequestUrl() + "dataSourceInfoHandler/getSQLInfoList",
		root : 'list',
		idProperty : 'ID',
		totalProperty : 'count',
		messageProperty : 'msg',
...
function showErrorWin(id) {
	...
	var url = getHandlerRequestUrl()+ "dataSourceInfoHandler/getSQLErrorInfo?id=" + id;
	var msgTip = Ext.MessageBox.show({
		title : '提示',
		width : 250,
		msg : '正在加载数据...'
			...
			.request({
				...
				success : function(response, options) {
					...
					if (result.success) {
						...
						Ext.Msg.alert('提示', "数据加载异常" + result.msg);
					}
				},
				failure : function(response, options) {
					var result = Ext.util.JSON.decode(response.responseText);
					msgTip.hide();
					Ext.Msg.alert('提示', "数据加载异常" + result.data);
				}
			});
...
//根据条件查询数据
function queryFunc(){
	var gridStore=Ext.getCmp("sqlInfoGrid").getStore();
	gridStore.baseParams = {};
	Ext.apply(gridStore.baseParams,{
		start : 0,
		limit : pageSize,
		sqlType :Ext.getCmp("sqlType_combo").getValue()
	});
	gridStore.load({
