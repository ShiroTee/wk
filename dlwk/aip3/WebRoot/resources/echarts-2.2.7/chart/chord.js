...
define(function(require)
{
	...
	// Source data matrix
	/**
	 * target -1--2--3--4--5- 1| x x x x x 2| x x x x x 3| x x x x x source 4| x
	 * x x x x 5| x x x x x
	 * 
	...
	Chord.prototype =
	{
			...
			for (var i = 0, l = series.length; i < l; i++) {
				if (series[i].type === this.type) {
					if (series[i].insertToSerie) {
						...
						if (chordSeriesGroups[mainSerie.name]
		...
		{
			type = type || 'normal';
			return [ (edge.itemStyle && edge.itemStyle[type]),
		...
		_buildChords : function(series)
		{
			...
			for (var i = 0; i < series.length; i++) {
				...
				if (this.selectedMap[serie.name]) {
					...
					// matrix 表示边
					if (serie.matrix) {
						graph = this._getSerieGraphFromDataMatrix(serie,
								mainSerie);
					}
					// links 表示边
					else if (serie.links) {
						graph = this._getSerieGraphFromNodeLinks(serie,
								mainSerie);
					}
					// 过滤输出为0的节点
			...
			if (!mainSerie.ribbonType) {
				...
				mainGraph.eachNode(
						function(node)
						{
							var queryTarget = this._getNodeQueryTarget(
									mainSerie, node);
							var symbolSize = this.query(queryTarget,
									'symbolSize');
							if (max === min) {
			...
			var showLabel = this
					.query(mainSerie, 'itemStyle.normal.label.show');

			if (mainSerie.ribbonType) {
				this._buildSectors(mainSerie, 0, mainGraph, mainSerie, graphs);
				if (showLabel) {
					this._buildLabels(mainSerie, 0, mainGraph, mainSerie,
							graphs);
				}
				...
				this
						._buildNodeIcons(mainSerie, 0, mainGraph, mainSerie,
								graphs);
				if (showLabel) {
					this._buildLabels(mainSerie, 0, mainGraph, mainSerie,
							graphs);
				...
				for (var i = 0, j = 0; i < series.length; i++) {
					if (this.selectedMap[series[i].name]) {
						this._buildEdgeCurves(series, i, graphs[j++],
								mainSerie, mainGraph);
					}
				}
			}
		...
		_initHoverHandler : function(series, graphs)
		{
			...
			mainGraph.eachNode(function(node)
			{
				...
				{
					console.log(node);
					node.shape.style.color="#2091CF";//68BC31
					if(node.data.father.length==0)
					{
						var inEdes=node.inEdges;
						for(var i=0;i<inEdes.length;i++)
						{
							var e=inEdes[i].node1;
							e.shape.style.color="#68BC31";
						}
					}
					else
					{
						node.shape.style.color="#68BC31";//68BC31
						for(var n=0;n<node.data.father.length;n++)
						{
							var pid=node.data.father[n];
							var inEdes=node.inEdges;
							for(var i=0;i<inEdes.length;i++)
							{
								var e=inEdes[i].node1;
								...
								if(pid==e.data.id)
								{
									e.shape.style.color="#2091CF";//
									continue;
								}
								for(var x=0;x<e.data.father.length;x++)
								{
									if(node.data.id==e.data.father[x])
									{
										e.shape.style.color="#68BC31";
										node.shape.style.color="#AF4E96";//68BC31
									}
								}
							}
						}
					...
					mainGraph.eachNode(function(n)
					{
						...
						if (n.labelShape) {
							...
							for (var i = 0; i < n.inEdges.length; i++) {
								n.inEdges[i].shape.style.strokeColor = "red";//
							}
					...
					for (var i = 0; i < graphs.length; i++) {
						for (var j = 0; j < graphs[i].edges.length; j++) {
							...
							e.shape.style.opacity = self.deepQuery(queryTarget,
									'opacity') * 0.1;
							e.shape.modSelf();
						}
					}
				...
				node.shape.onmouseout = function()
				{
					mainGraph.eachNode(function(n)
					{
						...
						n.shape.style.color = "#3a87ad";
						for (var i = 0; i < n.inEdges.length; i++) {
							n.inEdges[i].shape.style.strokeColor = "black";
						}
						if (n.labelShape) {
		...
		_buildSectors : function(serie, serieIdx, graph, mainSerie)
		{
			...
			graph.eachNode(function(node)
			{
				...
				// 默认使用 category 分类颜色
				var color = category ? this.getColor(category.name) : this
						.getColor(node.id);
				...
				sector.highlightStyle.lineWidth = this.deepQuery([ node.data,
						mainSerie ], 'itemStyle.emphasis.borderWidth');
				sector.style.strokeColor = this.deepQuery([ node.data,
						mainSerie ], 'itemStyle.normal.borderColor');
				sector.highlightStyle.strokeColor = this.deepQuery([ node.data,
						mainSerie ], 'itemStyle.emphasis.borderColor');
				...
				ecData.pack(sector, serie, serieIdx, node.data, node.rawIndex,
						node.id,
						// special
						node.category);
		...
		_buildNodeIcons : function(serie, serieIdx, graph, mainSerie)
		{
			...
			graph.eachNode(function(node)
			{
				...
				var queryTarget = this
						._getNodeQueryTarget(mainSerie, node.data);

				var category = this._getNodeCategory(mainSerie, node.data);
				var color = this.deepQuery(queryTarget,
						'itemStyle.normal.color');
				if (!color) {
					color = category ? this.getColor(category.name) : this
							.getColor(node.id);
				}
				var iconShape = new IconShape(
				{
					...
					style :
					{
						...
						lineWidth : this.deepQuery(queryTarget,
								'itemStyle.normal.borderWidth'),
						strokeColor : this.deepQuery(queryTarget,
				...
				ecData.pack(iconShape, serie, serieIdx, node.data,
						node.rawIndex, node.id,
						// special
						node.category);
		...
		_buildLabels : function(serie, serieIdx, graph, mainSerie)
		{
			...
			// );
			var rotateLabel = this.query(mainSerie,
					'itemStyle.normal.label.rotate');
			var labelDistance = this.query(mainSerie,
			...
			graph
					.eachNode(
							function(node)
							{
								var startAngle = node.layout.startAngle
										/ Math.PI * 180 * sign;
								var endAngle = node.layout.endAngle / Math.PI
										* 180 * sign;
								var angle = (startAngle * -sign + endAngle
										* -sign) / 2;
								angle %= 360;
								...
								var distance = 0;
								if (mainSerie.ribbonType) {
									distance = mainSerie.showScaleText ? 35 + labelDistance
											: labelDistance;
								} else {
								...
								var labelShape =
								{
									...
									style :
									{
										text : node.data.label == null ? node.id
												: node.data.label,
								...
								if (rotateLabel) {
									labelShape.rotation = isRightSide ? angle
											: Math.PI + angle;
									if (isRightSide) {
										labelShape.style.x = radius[1]
												+ distance;
									} else {
										labelShape.style.x = -radius[1]
												- distance;
								...
								// zrender/Text并没有textColor属性，ctx
								// fillStyle使用的是color
								labelShape.style.color = this
										.deepQuery([ node.data, mainSerie ],
												'itemStyle.normal.label.textStyle.color')
										|| '#000000';
								labelShape.style.textFont = this
										.getFont(this
												.deepQuery([ node.data,
														mainSerie ],
		...
		_buildRibbons : function(series, serieIdx, graph, mainSerie)
		{
			...
			graph.eachEdge(function(edge, idx)
			{
				...
				var queryTargetEmphasis = this._getEdgeQueryTarget(serie,
						edge.data, 'emphasis');
				var ribbon = new RibbonShape(
				{
					...
					style :
					{
						...
						strokeColor : this
								.deepQuery(queryTarget, 'borderColor'),
					...
					highlightStyle :
					{
						brushType : 'both',
						opacity : this
								.deepQuery(queryTargetEmphasis, 'opacity'),
						lineWidth : this.deepQuery(queryTargetEmphasis,
								'borderWidth'),
						strokeColor : this.deepQuery(queryTargetEmphasis,
								'borderColor')
					}
				});
				var node1, node2;
				...
				ecData.pack(ribbon, serie, serieIdx, edge.data,
			...
			graph
					.eachEdge(
							function(e, idx)
							{
								...
								var queryTarget = this._getEdgeQueryTarget(
										serie, e.data);
								var queryTargetEmphasis = this
										._getEdgeQueryTarget(serie, e.data,
												'emphasis');
								...
								var curveShape = new BezierCurveShape(
								{
									...
									style :
									{
										...
										strokeColor : this.deepQuery(
												queryTarget, 'color'),
										opacity : this.deepQuery(queryTarget,
												'opacity')
					...
					var textShape = new TextShape(
							{
								...
								style :
								{
									x : isRightSide ? radius[1]
											+ this.scaleLineLength + 4
											: -radius[1] - this.scaleLineLength
													- 4,
									y : 0,
		...
		reformOption : function(opt)
		{
			...
			opt = _merge(_merge(opt ||
			{}, this.ecTheme.chord), ecConfig.chord);
			opt.itemStyle.normal.label.textStyle = this
					.getTextStyle(opt.itemStyle.normal.label.textStyle);
			this.z = opt.z;
