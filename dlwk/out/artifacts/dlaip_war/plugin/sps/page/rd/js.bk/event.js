...
function createDetailForm()
{
	...
	var form = new Ext.FormPanel(
	{
		...
		{
			name : 'secrLv',mapping:function(data)
			{
...
function FileListGrid()
{
	...
	var gridStore = new Ext.data.Store(
	{
		...
		proxy : new Ext.data.HttpProxy(
		{
			url : getHandlerRequestUrl() + "serviceInfoHandler/getFileServiceByResourceId?resourceId="+resourceId
		})
	});
	...
	var sm = new Ext.grid.CheckboxSelectionModel({
		...
		singleSelect : true
			...
			var gridStore = new Ext.data.Store(
			{
				...
				{
					url : getHandlerRequestUrl()
							+ "serviceInfoHandler/getWebServiceByResourceId?resourceId="+resourceId
				})
			});
			var toolbar = new Ext.Toolbar();
			toolbar.addButton({
...
function deleteIntoService(grid)
{
	...
	if(list.length!=0)
	{
		var url=getHandlerRequestUrl()+"resourceCatalogueInfoHandler/deleteServiceInfo?routeId="+list[0];
...
function updateStatus(value){
	...
	Ext.MessageBox.confirm("提示", "你确定要"+str+"这条数据吗?", function(btnId) {
		if (btnId == 'yes') {
			...
			var url=getHandlerRequestUrl()+"resourceCatalogueInfoHandler/updateStatus";
			Ext.Ajax.request({
				url : url,
				method : 'POST',
				params:{
...
function updateAllStopOrStart(value){
	...
	Ext.MessageBox.confirm("提示", "你确定要"+str+"这些数据吗?", function(btnId) {
		if (btnId == 'yes') {
			...
			var url=getHandlerRequestUrl()+"resourceCatalogueInfoHandler/updateAllStopOrStart";
			Ext.Ajax.request({
				url : url,
				method : 'POST',
				params:{
