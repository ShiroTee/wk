...
function init() {
	...
	var fields = [ "id", 'type', 'typeName', 'name', 'imageUrl','appId' ];
	...
	var gridStore = new Ext.data.JsonStore({
		...
		url : getHandlerRequestUrl()
				+ "backgroundImageHandler/listBackgroundImage",
		root : 'list',
		totalProperty : 'count',
		messageProperty : 'msg',
		fields : fields
		...
		buttons : [
				{
					...
					handler : function() {
						...
						var appIds = Ext.getCmp("lovcombo_2").getValue();
						Ext.getCmp("editForm").getForm().doAction(
								'submit',
								{
									url : getHandlerRequestUrl()
											+ "backgroundImageHandler/save?appIds=" + appIds,
									method : 'POST',
									headers : {
										"Accept" : "text/json"
									},
...
function doDelete(id) {
	...
	Ext.Msg.confirm("", "请确认将删除此" + (id.split(",").length) + "项壁纸？",
			function(r) {
				if (r != "yes")
					return;
				Ext.Ajax.request({
					url : getHandlerRequestUrl() + 'backgroundImageHandler/remove',
					params : {
						id : id
					},
					method : 'POST',
