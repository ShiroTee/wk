...
function uploadSuccess(file, serverData) {
	try {
		var progress = new FileProgress(file,  this.customSettings.upload_target);
		var map =  eval("("+serverData+")")
		var fileName = file.name;
		var list=map[fileName];
		if(list[0]=='false' || list[0]==false){
            addFileInfo(file.id,list[1],'red');
            fileuploadProblem=fileuploadProblem+1;
        }else {
            addFileInfo(file.id,"文件上传完成",'green');
        }
...
function addFileInfo(fileId,message,color){
	var row = document.getElementById(fileId);
	row.cells[2].innerHTML = "<font color='"+color+"'>"+message+"</font>";
}
...
function uploadComplete(file) {
	try {
		...
		if (this.getStats().files_queued > 0) {
			...
			if(fileuploadProblem>0){
                progress.setStatus("<font color='red'>所有文件上传完毕,但还是存在操作失败文件!</b></font>");
            }else{
