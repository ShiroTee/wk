var dataSrc = PROJECT_ROOT + "/app/http/dms/jobDoHandler/getJobEchartsData";
var chartData;
var myChart;
...
$(function() {
		...
		getPieData(1,dataSrc,'作业执行情况统计图');
	});
...
function selectShowData() {
    ...
    getPieData(1,PROJECT_ROOT + "/app/http/dms/jobDoHandler/getJobEchartsData?startTime="+$("#param_startdate").val()+"&endTime="+$("#param_enddate").val(),'作业执行情况统计图');
}
...
function eConsole(param) {
	if(show_level ==1){
		if (confirm("确认下钻？")) {
			...
			var temp_url = PROJECT_ROOT + "/app/http/dms/jobDoHandler/getChildJobEchartsData?startTime="
			+$("#param_startdate").val()+"&endTime="+$("#param_enddate").val()+"&wbj_mc="+encodeURI(encodeURI(wbj_mc));
            getPieData(2,temp_url,wbj_mc+"-作业执行情况统计图"); 
        }
	}else{
		if (confirm("确认上钻？")) {
		        getPieData(1,PROJECT_ROOT + "/app/http/dms/jobDoHandler/getJobEchartsData?startTime="
		        	+$("#param_startdate").val()+"&endTime="+$("#param_enddate").val(),'作业执行情况统计图');
		}
	}
}
