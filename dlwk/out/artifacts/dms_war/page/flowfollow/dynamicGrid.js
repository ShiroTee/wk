...
var resultTableId = 'resultTable';
var idNum = 1;
//var queryFormCmp = Ext.getCmp(queryFormId);// 查询面板FormPanel的ID
var resultTable = Ext.getCmp(resultTableId);// 结果面板GridPanel的ID
...
var addBtnFunc = function(text, iconCls, handler)
{
	...
	queryFormCmp.addButton(btnItem);
}
...
Ext.onReady(function()
{
	...
	// 动态生成GridPanel
	var dynamicGrid = function(queryFormId, resultTableId)
	{
...
//					}
		...
		queryFormCmp = new Ext.FormPanel(
		{
			id : queryFormId,
			name : queryFormId,
			monitorResize : true,
			...
			labelAlign : 'right',
			layout : 'column',
			style : "overflow:hidden; height:92px;",
...
//			view : groupingView,
		...
		{	
			...
			items :
			[
					tipCmp, queryFormCmp, resultTable
			]
		});
	...
	initPage = function(response)
	{
		...
		if (success)
		{
			...
			// 重新设置Id,防止销毁不完全,导致各种异常出现
			queryFormId = 'queryForm' + idNum;
			resultTableId = 'resultTable' + idNum;
			idNum++;

			// 初始化Grid
			dynamicGrid(queryFormId, resultTableId);
	...
	queryFunc = function()
	{
		...
		resultStore.baseParams = queryFormCmp.getForm().getValues();
...
//			alert(operType)
			...
			} else
			{
				...
				nextColParam = colParamMap.data[0];
				...
				success : function(response)
				{
					...
					} else
					{
						...
						if (result == '{}')
						{
							...
							queryStep = currStep;
							queryParam = queryParamMap.get(currStep);
							alert('不能继续钻取了');
							var nextColParamnew='';
							if(nextColParam=='transGo')nextColParamnew='jobGo';
							if(nextColParam=='stepGo')nextColParamnew='transGo';
							queryColParam = nextColParamnew;
							return;
						} else
						{
							// 销毁Grid,重新构造
							queryFormCmp.destroy();
							resultTable.destroy();
