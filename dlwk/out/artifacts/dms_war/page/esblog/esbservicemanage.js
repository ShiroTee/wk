...
// 解决日期控件在IE浏览器下面上下显示不全的BUG
		Ext.override(Ext.menu.Menu,
	{
		...
		var resultStoreFields = [ 'SERVICE_ID', 'SERVICE_NAME','TYPE_ID', 'TYPE_NAME'];
		...
		},
{
		...
		/** *****以下为注册各页面组件方法******************************* */
		// 提示标签提示的内容;
		setTip(" 维护服务信息，可以添加、修改和删除服务信息。根据服务ID、服务名称和服务分类，查询相应服务信息。");
...
//		 利用setTbar(gridComponent,topToolbarItems)函数，设置表格的顶部工具栏，传入参数为：表格组件、顶部栏元素组
	...
	} else
	{
		...
		Ext.getCmp('SERVICE_ID_OLD').setValue(record.get('SERVICE_ID'));
		Ext.getCmp('typeComboxMax1').store.load();
		Ext.getCmp('SERVICE_ID').disable();
		...
		if (records.length == 0)
		{
			...
			for ( var i = 0; i < records.length; i++) {
				var rec = records[i];
				recordIdArr.push(rec.get('SERVICE_ID'));
				var ids = rec.get('SERVICE_ID');
			}
			...
			var dataQualityFormInfo = new Ext.FormPanel(
					{
						...
						},typeComboxMax1,
						{
							...
							name : 'SERVICE_ID',
							id : "SERVICE_ID",
							fieldLabel : '<span style="color:red;font-size:13pt;">*</span>服务ID'
						},						
						{
							xtype : 'textfield',
							width : 400,
							...
							name : 'SERVICE_NAME',
							id : "SERVICE_NAME",
							fieldLabel : '<span style="color:red;font-size:13pt;">*</span>服务名称'
						}],
						buttons : [
								{
									text : '保存',
