...
// 各种链接
var listUrl =PROJECT_ROOT+"app/http/dms/esbServiceManageHandler/getServiceType";
/**
 * 页面--转换操作跟踪情况列表展示
 */
...
// 解决日期控件在IE浏览器下面上下显示不全的BUG
		Ext.override(Ext.menu.Menu,
	{
		...
		var resultStoreFields = ['TYPE_ID', 'TYPE_NAME'];
		...
		},
{
		...
		/** *****以下为注册各页面组件方法******************************* */
		// 提示标签提示的内容;
		setTip(" 维护服务分类信息，可以添加、修改和删除服务分类。根据服务分类ID、服务分类名称，查询相应服务分类信息。");
...
//		 利用setTbar(gridComponent,topToolbarItems)函数，设置表格的顶部工具栏，传入参数为：表格组件、顶部栏元素组
	...
	} else
	{
		...
		Ext.getCmp('TYPE_ID_OLD').setValue(record.get('TYPE_ID'));
		Ext.getCmp('TYPE_ID').disable();
		...
		if (records.length == 0)
		{
			...
			for ( var i = 0; i < records.length; i++) {
				var rec = records[i];
				recordIdArr.push(rec.get('TYPE_ID'));
				var ids = rec.get('TYPE_ID');
			}
			...
			if (recordIdArr.length != 0)
			{
				Ext.Msg.confirm("提示", "确定要执行删除吗？", function(btn, text)
				{
					if (btn == "yes")
					{
						...
						{
							// 发送请求
							url : PROJECT_ROOT + "app/http/dms/esbServiceManageHandler/delServiceType",
							method : 'POST',
							params :
							{
			...
			var dataQualityFormInfo = new Ext.FormPanel(
					{
						...
						},
						{
							...
							name : 'TYPE_ID',
							id : "TYPE_ID",
							fieldLabel : '<span style="color:red;font-size:13pt;">*</span>服务分类ID'
						},						
						{
							xtype : 'textfield',
							width : 400,
							...
							name : 'TYPE_NAME',
							id : "TYPE_NAME",
							fieldLabel : '<span style="color:red;font-size:13pt;">*</span>服务分类名称'
						}],
						buttons : [
								{
									text : '保存',
									...
									handler : function() {
										var submitUrl,win;
										if(op_type =='add'){
										    submitUrl= PROJECT_ROOT + "app/http/dms/esbServiceManageHandler/addServiceType";
										}else{
											submitUrl= PROJECT_ROOT + "app/http/dms/esbServiceManageHandler/updateServiceType";
										}
