 ...
 * @copyright Free for all use and modification. The author and copyright must
 *            be remain intact here.
 * 
 * @class Ext.form.MultiSelectField
 * @extends Ext.form.TriggerField
 */
Ext.form.MultiSelectField = Ext
		.extend(
				Ext.form.TriggerField,
				{
					...
					displayField : 'text',
					needcheck:false,
					checkmethod:function(){return false;},

					initComponent : function() {
						Ext.QuickTips.init();
						Ext.form.MultiSelectField.superclass.initComponent
								.call(this);
					...
					onRender : function(ct, position) {
						...
						if (this.menu == null) {
							...
							this.menu.on('hide', function() {
								if (this.tmp != this.lastSelectionText
					...
					onTriggerClick : function() {
						...
						if(this.menu != null && this.menu.items.length == 0)
						{
					...
					getValue : function() {
						if (this.hiddenField) {
							...
							return Ext.form.MultiSelectField.superclass.getValue
									.call(this);
						}
					},
					...
					setValues : function(keys) {
						...
						for ( var i = 0; i < keys.length; i++) {
							if (keys[i] != undefined && keys[i] != '') {
								...
								var item = this.store.query(this.valueField,
										keys[i]).items[0];
								if (item != undefined) {
									value += (value != '' ? this.valueSeparator
											: '')
											+ item.data[this.valueField];
									text += (text != '' ? this.textSeparator
											: '')
											+ item.data[this.displayField];
								}
							}
						}
					...
					selPush : function(key) {
						...
						var keys = this.value == '' ? new Array() : this.value
					...
					onDestroy : function() {
						...
						Ext.form.MultiSelectField.superclass.onDestroy
								.call(this);
					},
							...
							for (i = 0; i < sel.length; i++) {
								try {
									var value = this.store.query(
											this.valueField, sel[i]).items[0].data[this.valueField];
