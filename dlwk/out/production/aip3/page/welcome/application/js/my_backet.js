...
function assembleRoutes(data){
	...
	for(var i=0;i<data.length;i++){
		...
		+ 		'<a target="_blank" href="' + ctx + '/mdp/welcome/resourceCatalog/details.html?routeid=' + content.resourceId + '" class="cart_name">' + content.resourceName + '</a>'
		+ 		'<div class="res_labs">'
		+ 			'<span class="res_lab">' + content.provider.orgName + '</span>'
		+ 		'</div></div></div>'
		+ '<div class="cart_col3">'
		+ 	'<a href="javascript:void(0);" onclick="deleteSingleService($(this))" sessiontype="route" routeid = "' + content.resourceId + '" class="cart_del"><i class="ico i205 cart_delIco"></i>删除</a>'
		+ '</div></label>';
	}
...
function assembleAPIs(data){
	...
	for(var i=0;i<data.length;i++){
		...
		+ '</div>'
		+ '<div class="cart_col3">'
		+ '<a href="javascript:void(0);" class="cart_del"><i class="ico i205 cart_delIco"></i>删除</a>'
		+ '</div>'
		+ '</label>';
	}
