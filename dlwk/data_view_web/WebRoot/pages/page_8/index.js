...
//						+ "\x3cbr\x3e坐标:" + a.data.value[0]+','+a.data.value[1]
	...
	ringChart : function() {
		...
		leftChart1 = echarts.init(document.getElementById("cuser1")), d = [], e = {
			...
			tooltip : {
				formatter : function(a, c, d) {
					return '信息资源共享度' + "\x3cbr\x3e无条件共享:" + wgx
							+ "\x3cbr\x3e有条件共享:" + ygx + "\x3cbr\x3e不予共享:"
							+ bgx
				}
			},
			toolbox : {
...
//								[ 1, 'lime' ] ],
	...
	ringChart1 : function() {
		...
		leftChart2 = echarts.init(document.getElementById("cuser2")), d = [], e = {
			...
			tooltip : {
				formatter : function(a, c, d) {
					return '信息资源开放度' + "\x3cbr\x3e普遍开放:" + wgx
							+ "\x3cbr\x3e依申请开放:" + ygx + "\x3cbr\x3e不开放:" + bgx
				}
			},
			toolbox : {
				right : 16,
...
//								[ 1, 'lime' ] ],
	...
	right3 : function() {
		...
		leftChart3 = echarts.init(document.getElementById("right3")), d = [], e = {
			color : "#86D854 #6AA9EB #FF7E9F #F3D25D #EE1604".split(" "),
			tooltip : {
				trigger : 'item',
				position : [ 10, 10 ],
